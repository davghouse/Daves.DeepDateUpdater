...
@"SELECT
    ...
    convert(bit, case when system_type_id in (40, 61, 42, 43, 58) then 1 else 0 end) isDateType
FROM sys.columns
...
WHERE parent_object_id IN (SELECT object_id FROM sys.tables)";
        ...
        public virtual IReadOnlyList<Column> QueryColumns()
            => Query(ColumnQuery,
                r => new Column(r["tableId"], r["name"], r["columnId"], r["isNullable"], r["isIdentity"], r["isComputed"], r["isDateType"]))
            .ToReadOnlyList();
